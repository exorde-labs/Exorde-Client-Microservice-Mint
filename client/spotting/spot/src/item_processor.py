...
async def processing_logic(app, current_item):
    ...
    with tracer.start_as_current_span("process_item") as processing_span:
        try:
            ...
            processed_item = process(current_item, app["static_configuration"]["lab_configuration"], app["live_configuration"]["max_depth"])
            ...
            with tracer.start_as_current_span("send_processed_item") as send_span:
                ...
                if target:
                    async with ClientSession() as session:
                        async with session.post(target, json=processed_item) as response:
